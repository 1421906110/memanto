...
class TestF2AllowlistNotEnforced:
    ...
    def test_update_memory_accepts_non_allowlisted_status(self):
        ...
        # Patch at the CLASS level so the MemoryReadService created inside
        # update_memory also sees the mock.  Accept positional args because
        # get_memory is called as get_memory(memory_id, namespace).
        def fake_get(_self, _memory_id, _namespace):
            return {
                "id": "f2-test-id",
                "title": "F2 original",
        ...
        with patch.object(MemoryReadService, "get_memory", fake_get):
    ...
    def test_allowlist_defined_but_nowhere_imported(self):
        """Assert the constant is unreferenced outside ``constants.py``.

        Walks every Python file under ``memanto/`` and looks for the name
        ``ALLOWED_UPDATE_FIELDS`` in a *Load* context (i.e. reading the
        value, not defining it).  The definition site in ``constants.py``
        is excluded by filename.
        """
        import ast
        import os
        ...
        for dirpath, _dirs, fnames in os.walk(memanto_root):
            for fn in fnames:
                if not fn.endswith(".py"):
                    continue
                # Skip the definition module itself
                if fn == "constants.py":
                    continue
                ...
                for node in ast.walk(tree):
                    if (
                        ...
                        and node.id == "ALLOWED_UPDATE_FIELDS"
...
class TestF4UpdateAtomicity:
    ...
    def test_upload_failure_after_delete_causes_data_loss(self):
        ...
        # Patch at the CLASS level so update_memory's internal
        # MemoryReadService also sees the stub.  Accept positional args.
        def fake_get(_self, _memory_id, _namespace):
            return {
                "id": "f4-test-id",
        ...
        calls = [
            ...
            if call[0] in ("documents.delete", "documents.upload")
        ]
    ...
    def test_no_recovery_keywords_in_update_memory(self):
        ...
        Uses AST to extract only the ``update_memory`` method body
        (line-based, not character-based) and checks for keywords that
        would indicate a rollback / compensating action.
        """
        import ast
        import os
        ...
        # Use line-based extraction (not character slicing)
        lines = source.splitlines(keepends=True)
        method_lines = lines[update_node.lineno - 1 : update_node.end_lineno]
...
class TestChainIntegrity:
    """Runs short checks for all four defects with ``subtests``.

    Gives a single-pass/fail view of the whole chain in CI output.
    """
...
def _ttl_else_branch_still_exists() -> bool:
    ...
    Uses AST to scope the check to ``_filter_expired_memories`` only,
    so we don't match an ``else:`` from a different method.
    """
    import ast
    import os
    ...
    method_node = next(
        (n for n in ast.walk(tree)
         if isinstance(n, ast.FunctionDef) and n.name == "_filter_expired_memories"),
        None,
    )
    if method_node is None:
        return True  # method missing — assume defect present

    lines = source.splitlines(keepends=True)
    meth_lines = lines[method_node.lineno - 1 : method_node.end_lineno]
    method_source = "".join(meth_lines)

    # Defect pattern: has isinstance(expires_at, str) check but no
    # datetime/expiry check in the else branch.
    return (
        "isinstance(expires_at, str)" in method_source
        and "else:" in method_source
    )


def _no_recovery_in_update_memory() -> bool:
    """Return True when ``update_memory`` method lacks recovery keywords.

    AST-scoped to the method body, not the whole file.
    """
    import ast
    import os
